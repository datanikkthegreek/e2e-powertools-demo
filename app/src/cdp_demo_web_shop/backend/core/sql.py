...
from ._defaults import ClientDependency
...
class _SqlDependency(LifespanDependency):
    ...
    def __call__(request: Request, ws: ClientDependency) -> Sql:
        # Use the app-level service principal client (profile/SP credentials),
        # not the per-user OBO client. The analytics counts are aggregate and
        # not user-scoped, and this avoids requiring an X-Forwarded-Access-Token
        # (which only exists in a deployed Databricks App, not local dev).

...
CREATE OR REFRESH STREAMING TABLE fact_purchase_line (
  purchase_line_id STRING NOT NULL COMMENT 'Canonical UUID primary key identifying this individual line item. Derived from the binary Lakebase purchase_lines id.',
  ...
  CONSTRAINT pk_fact_purchase_line PRIMARY KEY (purchase_line_id),
  CONSTRAINT fk_fact_purchase_line_purchase FOREIGN KEY (purchase_id) REFERENCES fact_purchase(purchase_id),
  CONSTRAINT fk_fact_purchase_line_product FOREIGN KEY (product_id) REFERENCES dim_product(product_id)
)

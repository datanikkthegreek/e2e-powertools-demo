...
Source: the **Internet Archive** (archive.org), which hosts scanned/OCR'd
manufacturer manuals as directly-downloadable ("Text PDF") files behind a clean,
scriptable JSON API — no HTML-viewer scraping, no bot walls. Flow per tool:

  1. Query the IA advanced-search JSON API for the tool's model.
  2. STRICT match: the candidate item's title must contain the tool's exact model
     designation (spaces ignored) — this rejects sibling variants (e.g. a
     "GST 18 V-LI" manual will NOT be accepted for "GST 18V-LI S").
  3. Fetch the item metadata JSON, pick its PDF file, download it.
  4. Verify it is a real PDF (``%PDF`` header, non-trivial size, > 1 page) before
     it is allowed to be uploaded.
  5. Upload each verified PDF individually to the manuals/ Volume subfolder.
...
DIY). Free scriptable manual archives skew older, so several models legitimately
resolve to "unsourced" here — that is reported, not faked.
...
USER_AGENT = (
    ...
    "(Databricks demo; sourcing public Bosch manuals from archive.org)"
)
